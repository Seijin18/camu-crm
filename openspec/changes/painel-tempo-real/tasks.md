...
- [ ] 1.1 `camucrm/db.py`: `token_de_mudanca` retorna a string de 3 partes
      descrita no `design.md` (→ Requirement "Token de mudança como
      cursor").
- [ ] 1.2 `camucrm/painel/stream.py`: poller único (`asyncio.Task`) a cada
...
- [ ] 1.3 `camucrm/painel/server.py`: pool de conexão próprio do painel,
      `max_size` > 5; startup/shutdown do poller único.
- [ ] 1.4 `camucrm/painel/static/app.js`: cliente SSE via `fetch()` +
...
- [ ] 2.1 `tests/test_painel_stream.py`: token como comparação pura (sem
