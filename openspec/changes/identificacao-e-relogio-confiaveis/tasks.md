...
- [ ] 1.1 `camucrm/transport/evolution.py::receber`: descartar eventos de
      `status@broadcast` e `@broadcast` (devolve `None`, mesmo tratamento de
      ruído de protocolo) (→ Requirement "Broadcast e status não criam
      evento").
- [ ] 1.2 `camucrm/transport/evolution.py::receber`: recusar (logar e
...
- [ ] 2.1 `camucrm/transport/evolution.py::_timestamp`: clampar timestamp
...
- [ ] 2.2 `camucrm/transport/evolution.py::_timestamp`: clampar timestamp
...
- [ ] 3.1 `tests/test_transport.py`: evento de `status@broadcast` não
      cria/atualiza contato nem conversa (→ Requirement "Broadcast e status
      não criam evento").
- [ ] 3.2 `tests/test_transport.py`: evento de `@broadcast` não cria/atualiza
      contato nem conversa (→ Requirement "Broadcast e status não criam
      evento").
- [ ] 3.3 `tests/test_transport.py`: evento `@lid` sem PN confiável é
...
- [ ] 3.4 `tests/test_transport.py`: timestamp futuro (além da tolerância) é
      clampado a `agora()` antes de alimentar o `GREATEST` de
      `ultimo_inbound` (→ Requirement "Timestamp futuro não trava o relógio
      da conversa").
- [ ] 3.5 `tests/test_transport.py` (ou `test_ingest.py`): mensagem real
      ...
      conversa").
- [ ] 3.6 Suíte completa verde (`make test`).
