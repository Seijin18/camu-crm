...
- [ ] 1.1 `camucrm/summaries.py`: prompt determinístico montado em Python —
...
- [ ] 1.2 `camucrm/summaries.py`: `gerar()` chama `llm`, devolve JSON
...
- [ ] 1.3 `camucrm/summaries.py`: `PROMPT_VERSAO_RESUMO = "1"`; docstring
...
- [ ] 2.1 `camucrm/db.py`: tabela `resumos_conversa` em `SCHEMA` — colunas
...
- [ ] 2.2 `camucrm/db.py`: leitura/escrita de resumo — checagem de cache
      antes da chamada ao LLM; `?forcar=true` usa `ON CONFLICT ... DO
      UPDATE` (→ Requirement "Cache por versão de prompt e mensagem").
- [ ] 2.3 `camucrm/db.py`: estender `purgar_mensagens_antigas` (`db.py:878`)
...
- [ ] 3.1 `camucrm/painel/api.py`: `POST /api/resumos` (gera, nunca `GET`,
      nunca automático) (→ Requirement "Geração só ao clicar, nunca
      automática").
- [ ] 3.2 `camucrm/painel/static/*`: ordem de tela — cabeçalho, fatos com
...
- [ ] 3.3 `CLAUDE.md`: editar "exatamente dois lugares" para três, com a
      justificativa da folha (fora de `openspec/` — executor do change
      edita, não este agente de planejamento).

## 4. Testes

- [ ] 4.1 `tests/test_summaries.py`: prompt contém evidência literal e
      ...
      tela").
- [ ] 4.2 `tests/test_summaries.py`: guardas por `ast.parse` — `rules/` não
...
- [ ] 4.3 `tests/integration/`: índice único de `resumos_conversa` torna a
