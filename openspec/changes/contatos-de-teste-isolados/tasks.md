...
- [ ] 2.2 Rota `GET /api/conversas`: propaga o parâmetro de modo (→
      Requirement "Leitura agregada exclui teste por padrão").
- [ ] 2.3 `camucrm/metrics.py::conversao` / `metricas_chave` (§14): parâmetro
      de modo, exclui teste por padrão (→ Requirement "Leitura agregada
      exclui teste por padrão").
- [ ] 2.4 `camucrm/metrics.py::tempo_por_estagio`: parâmetro de modo (→
      Requirement "Leitura agregada exclui teste por padrão").
- [ ] 2.5 `camucrm/metrics.py::saude_taxonomia` / `distribuicao_objecoes` /
      `objecao_por_estagio`: parâmetro de modo (→ Requirement "Leitura
      agregada exclui teste por padrão").
- [ ] 2.6 `camucrm/metrics.py::padrao_correcoes`: parâmetro de modo (→
      Requirement "Leitura agregada exclui teste por padrão").
- [ ] 2.7 `camucrm/metrics.py::retorno_por_followup`: parâmetro de modo (→
      Requirement "Leitura agregada exclui teste por padrão").
- [ ] 2.8 `camucrm/metrics.py::onde_morrem`: parâmetro de modo (→
      Requirement "Leitura agregada exclui teste por padrão").
- [ ] 2.9 `camucrm/metrics.py::ab_rascunhos`: parâmetro de modo (→
      Requirement "Leitura agregada exclui teste por padrão").
- [ ] 2.10 `GET /api/o-que-funciona`: propaga o parâmetro de modo a todas as
...
- [ ] 3.1 `camucrm/cli.py::cmd_fila`: `--incluir-teste`/`--somente-teste` (→
      Requirement "Leitura agregada exclui teste por padrão").
- [ ] 3.2 `camucrm/painel/static/*`: toggle "Modo teste" no topo do painel,
      ao lado do token/operador (→ Requirement "Modo teste nunca mistura as
      duas visões na mesma tela").
- [ ] 3.3 `camucrm/painel/static/*`: botão no detalhe da conversa
      ("marcar/desmarcar contato de teste") (→ Requirement "Marcação de
      teste é sempre manual e registrada").
- [ ] 3.4 `camucrm/painel/api.py`: propagar o parâmetro de modo (toggle) em
...
- [ ] 4.1 `tests/test_contatos_teste.py`: para cada função da seção 2, com
...
- [ ] 4.2 `tests/test_contatos_teste.py`: extração/regras/rascunho/resumo
      ...
      afeta processamento").
- [ ] 4.3 `tests/test_contatos_teste.py`: `marcar_contato_teste` grava linha
      em `correcoes`, não em `marcos_manuais` (→ Requirement "Marcação de
      teste é sempre manual e registrada").
- [ ] 4.4 Suíte completa verde (`make test`).
