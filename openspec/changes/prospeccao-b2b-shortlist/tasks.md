...
- [x] 1.1 `camucrm/db.py`: tabela `prospeccoes` (`design.md`) na constante
      ...
      contatos/conversas").
- [x] 1.2 `db.importar_prospeccoes(linhas: list[dict]) -> ResumoImportacao`:
      ...
      em silêncio").
- [x] 1.3 `db.listar_prospeccoes(*, zona=None, bairro=None, nota_minima=None,
...
- [x] 1.4 `db.marcar_prospeccao_aberta(prospeccao_id, *, por)`: grava
      `aberto_em`/`aberto_por` (→ Requirement "Abertura de link é
      registrada").
- [x] 1.5 `db.prospeccao_por_telefone_hash(hash) -> ProspeccaoRegistro | None`.

## 2. Ingestão

- [x] 2.1 `camucrm/ingest.py::ingerir`: antes de `upsert_contato`, consulta
...
- [x] 3.1 `docs/mensagem-prospeccao.md` — template com `{nome}`, path
...
- [x] 3.2 Função pura (`camucrm/prospeccao.py` ou dentro de
...
- [x] 4.1 `POST /api/prospeccao/importar` (multipart/form-data, CSV) →
      `views.resumo_importacao_para_json`.
- [x] 4.2 `GET /api/prospeccao?zona=&bairro=&nota_minima=&tier=&nao_convertidas=`
      → lista com link/mensagem prontos por linha.
- [x] 4.3 `POST /api/prospeccao/{id}/abrir` — corpo `{por}`, grava
      `aberto_em`/`aberto_por`.

## 5. Painel — front

- [x] 5.1 Aba nova "Importar prospecção": upload de CSV, relatório do
      resultado (N novos, M atualizados, K inválidos com motivo).
- [x] 5.2 Aba nova "Prospecção": tabela com os campos da planilha + filtros
...
- [x] 5.3 **Nunca aparece em kanban/fila/conversas/métricas** — confirmar
      que nenhuma rota existente foi tocada para incluir `prospeccoes` (→
      Requirement "Shortlist separada de contatos/conversas").

## 6. Testes

- [x] 6.1 `tests/test_prospeccao.py`: importar CSV novo cria linhas;
...
- [x] 6.2 Teste de conversão: contato/conversa real criada via `ingest.ingerir`
