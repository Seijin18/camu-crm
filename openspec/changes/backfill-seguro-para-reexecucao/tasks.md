...
- [x] 1.1 `camucrm/backfill.py::importar_conversas`: gerar `externa_id`
...
- [x] 2.1 `camucrm/extraction/extractor.py`/`backfill.py`: dividir
...
- [x] 2.2 `camucrm/db.py` (`mensagens_novas` ou consumo em `Extrator`):
...
- [x] 3.1 `camucrm/pipeline.py::_trilha_de_backfill`: checar por par `(de,
...
- [x] 4.1 `tests/test_backfill.py`: reimportar o mesmo dump duas vezes não
      duplica mensagem (→ Requirement "Reimportar dump sem externa_id não
      duplica mensagem").
- [x] 4.2 `tests/test_backfill.py` (teste de regressão, herdado do change
...
- [x] 4.3 `tests/test_backfill.py`: histórico de 1000+ mensagens não estoura
      numa única chamada de LLM (→ Requirement "Histórico grande não
      estoura numa chamada só").
- [x] 4.4 `tests/test_backfill.py`: mensagens com `id` de inserção divergente
      de `enviada_em` são lidas na ordem cronológica correta (→ Requirement
      "Ordem de leitura bate com enviada_em").
- [x] 4.5 `tests/test_pipeline.py` (ou equivalente): `_trilha_de_backfill`
