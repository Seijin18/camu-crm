...
- [ ] 1.3 `camucrm/transport/evolution.py::_texto_da_mensagem`: desembrulhar
      ...
      recursivamente").
- [ ] 1.4 `camucrm/transport/evolution.py::_texto_da_mensagem`: desembrulhar
...
- [ ] 1.5 Confirmar que um `.message` interno não reconhecido, dentro de
...
- [ ] 1.6 Atualizar o docstring de `_tipo_de_midia` para parar de descrevê-lo
...
- [ ] 2.2 Confirmar que `test_evento_sem_texto_e_ignorado` (reactionMessage)
      ...
      continua descartado").
- [ ] 2.3 `tests/test_transport.py`: texto puro dentro de `ephemeralMessage`
      ...
      recursivamente").
- [ ] 2.4 `tests/test_transport.py`: mídia sem legenda dentro de um envelope
      ...
      desembrulhado recursivamente").
- [ ] 2.5 `tests/test_transport.py`: `deviceSentMessage` com texto interno
...
- [ ] 2.6 `tests/test_ingest.py` ou extensão de `tests/test_e2e.py`: um
      ...
      fato").
- [ ] 2.7 Suíte completa verde (`make test`).
